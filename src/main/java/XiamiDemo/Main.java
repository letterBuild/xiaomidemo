...
public class Main {
	...
	public static void main(String[] args) {
		...
		//String url = "http://www.xiami.com/song/1772446367";
		//main.downLoadBySong(url);

		//main.downLoadByAlbum("http://www.xiami.com/album/2100200848");
		main.downLoadBySongList("http://www.xiami.com/song/playlist/id/1772446367/object_name/default/object_id/0");
	}

	void downLoadByAlbum(String albumUrl) {
		List<String> songsUrl = new ArrayList<String>();
		try {
	...
	void downLoadBySongList(String listUrl) {
		List<String> songsUrl = new ArrayList<String>();
		try {
			Document doc = Jsoup.connect(listUrl).followRedirects(true).timeout(10000)
					.userAgent("Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.0)").get();
			Elements songs = doc.select("td.song_name");
			for (Element e : songs) {
				System.out.println(e.text() + " : " + e.select("a").attr("abs:href"));
	...
	void downLoadBySong(Song song) {
		...
		helpDownload(albumPath, songName, downloadLocation);
		helpDownload(albumPath, songName, song.getPicUrl());
		helpDownload(albumPath, songName, song.getLyric());
	...
	void downLoadBySong(String songUrl) {
		...
		String downloadLocation = song.getDownloadLocation();
		String songName = song.getSongName();
		System.out.println("sonurl:"+songUrl);
		System.out.println("songName:"+songName+"  down:"+downloadLocation);
